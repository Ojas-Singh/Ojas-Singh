...
pub fn secondQuantizationOneBodyOperator(p:usize,q:usize,state:BitVec,state2:&BitVec) -> isize{
    let mut phase = 1;
    ...
    for m in 0..nslater {
        for n in m..nslater {
            for p in 0..M {
                for q in 0..M {
                    let mut phase = 0;
                    phase += secondQuantizationOneBodyOperator(2*p, 2*q, binstates[n].clone(),&binstates[m]);
                    phase += secondQuantizationOneBodyOperator(2*p+1, 2*q+1, binstates[n].clone(), &binstates[m]);
                    hamiltonian[m][n] += h[p][q]*phase as f64;

                    for r in 0..M {
                        for s in 0..M {
                                phase = 0;
                                phase += secondQuantizationTwoBodyOperator(2*p, 2*q, 2*r, 2*s,binstates[n].clone(), &binstates[m]);
                                phase += secondQuantizationTwoBodyOperator(2*p+1, 2*q+1, 2*r+1, 2*s+1, binstates[n].clone(),&binstates[m]);
                                phase += secondQuantizationTwoBodyOperator(2*p, 2*q+1, 2*r, 2*s+1, binstates[n].clone(), &binstates[m]);
                                phase += secondQuantizationTwoBodyOperator(2*p+1, 2*q, 2*r+1, 2*s, binstates[n].clone(), &binstates[m]);
