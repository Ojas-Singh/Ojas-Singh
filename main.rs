...
pub fn createbinarystatearray(state:Vec<isize>,m:usize) -> BitArray::<u64,U64> {
...
pub fn createslaterdeterminants(n:usize,m:usize,excite:String) -> Vec<BitArray::<u64,U64>> {
    ...
    for i in statesup {
        for j in &statesdown {
            let binstate = createbinarystatearray(mix(i.to_vec(),j.to_vec()), m as usize);
...
pub fn computeHamiltonianMatrix(binstates:Vec<BitArray::<u64,U64>>,v:Vec<Vec<Vec<Vec<f64>>>>,h:Vec<Vec<f64>>,M:usize)-> Vec<Vec<f64>> {
    ...
    for m in 0..nslater {
        for n in m..nslater {
            for p in 0..M {
                for q in 0..M {
                    let mut phase = 0.0;
                    phase += secondQuantizationOneBodyOperator(2*p, 2*q, &binstates[n],&binstates[m]);
                    phase += secondQuantizationOneBodyOperator(2*p+1, 2*q+1, &binstates[n], &binstates[m]);
                    hamiltonian[m][n] += h[p][q]*phase;
                    ...
                    for r in 0..M {
                        for s in 0..M {
                                ...
                                hamiltonian[m][n] += 0.5*v[p][r][q][s]*phase;
